...
import { supabase } from '@/lib/supabase'; // 👈 Наш ключ до бази
                            ...
                            <div className="flex flex-col space-y-4">
                                {items.map((item, idx) => (
                                    <motion.div 
                                        ...
                                        onClick={() => handleBooking(item.cat_id, item.srv_id)}
                    ...
                    >
                        <div className="w-full md:w-1/2">
                            <div className="relative aspect-[4/5] overflow-hidden rounded-[2.5rem] shadow-2xl border border-white/5">
                                <Image src={tattooMaster.image} alt={tattooMaster.name} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" />
                            </div>
                        </div>
                        <div className="w-full md:w-1/2 text-center md:text-left">
                            <p className="text-foxy-accent font-bold tracking-[0.3em] uppercase text-[10px] mb-4">{tattooMaster.role}</p>
                            <h2 className="font-playfair text-4xl md:text-5xl font-bold mb-8">{tattooMaster.name}</h2>
                    ...
                    {tattooImages.map((img: any, index: number) => {
                        ...
                        return (
                            <motion.div key={img.id} layout onClick={() => setLightboxIndex(index)} className={`relative group cursor-pointer overflow-hidden rounded-3xl bg-black/5 ${patterns[index % patterns.length]}`}>
                                <Image src={img.src} alt="Tattoo work" fill className="object-cover transition-transform duration-700 group-hover:scale-110" sizes="(max-width: 768px) 50vw, 25vw" />

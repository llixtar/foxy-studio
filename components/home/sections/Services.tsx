...
// Тільки топові послуги для швидкого прев'ю
const servicesData = [
  {
    id: 'tatuaz',
    ...
    items: [
      ...
      { name: 'Tatuaż minimalistyczny', price: '200 zł' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Przedłużanie paznokci', price: 'od 170 zł' },
    ]
  },
  {
    id: 'brwi',
    ...
    items: [
      ...
      { name: 'Koloryzacja + geometria', price: '90 zł' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Mega Volume (6D+)', price: '160 zł' },
    ]
  }
];
...
export default function Services() {
  ...
  const handleBooking = (itemName: string) => {
    console.log(`Резервуємо: ${itemName}`);
  };
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
          {servicesData.map((service, index) => (
            <motion.div 
              ...
              whileHover={{ y: -10 }}
              className={`relative flex flex-col group ${activeId === service.id ? 'z-30' : 'z-10'}`}
            >
              <div 
                className={`
                  relative h-56 md:h-72 rounded-3xl
                  border border-white/10
                  transition-all duration-300 shadow-xl
                  ${activeId === service.id ? 'ring-2 ring-foxy-accent' : ''}
                `}
              >
                ...
                <div className="absolute bottom-0 left-0 w-full p-3 md:p-4 flex flex-col items-center justify-center bg-black/30 backdrop-blur-md rounded-b-3xl z-10 min-h-[60px] border-t border-white/10">
                  ...
                  <motion.span 
                    ...
                    className="text-foxy-accent absolute right-3 hidden md:block"
                  >
                ...
                <AnimatePresence>
                  {activeId === service.id && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -10 }}
                      className="absolute top-[100%] left-0 w-full mt-2 overflow-hidden bg-black/60 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl z-20"
                    >

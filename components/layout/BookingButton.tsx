...
import BookingModal from '../booking/BookingModal'; // ПЕРЕВІР ШЛЯХ!
...
export default function BookingButton() {
  ...
  // --- МАГІЯ: СЛУХАЄМО СИГНАЛ ВІД ІНШИХ КОМПОНЕНТІВ ---
  useEffect(() => {
    const openModal = () => setIsModalOpen(true);
    
    window.addEventListener('openModalGlobal', openModal);
    return () => window.removeEventListener('openModalGlobal', openModal);
  }, []);
    ...
    <>
      ...
      >
        <button
          ...
          className="
            w-full py-4 px-8
            bg-foxy-accent/90 backdrop-blur-lg

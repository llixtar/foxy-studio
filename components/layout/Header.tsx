      ...
      >
        <div className="container mx-auto px-6 max-w-7xl">
          <div className="flex items-center justify-between">
            <Link href="/" className="flex items-center transition-transform hover:scale-105" onClick={closeMenu}>
              <img
                src="/assets/FOXY.svg"
                alt="Foxy Studio Logo"
                className={`h-16 md:h-24 w-auto transition-all duration-500 ${
                  !isPastHero ? 'invert brightness-0 drop-shadow-[0_0_0.5px_rgba(255,255,255,1)]' : 'drop-shadow-[0_0_0.3px_rgba(0,0,0,1)]'
                }`}
              />
            </Link>
        ...
        {isMenuOpen && (
          ...
          >
            ...
            <div className="flex h-24 items-center justify-between px-6 border-b border-foxy-text/5 shrink-0">
              ...
              <button onClick={closeMenu} className="p-2 text-foxy-text hover:text-foxy-accent transition-colors">
                ...
                item.type === 'dropdown' ? (
                  <div key={item.title} className="flex flex-col items-center w-full">
                    ...
                    <div className={`flex flex-col items-center gap-5 overflow-hidden transition-all duration-300 ${
                      ...
                      {item.items?.map((subItem) => (
                        <Link key={subItem.name} href={subItem.url} onClick={closeMenu} className="text-base text-foxy-text/70 hover:text-foxy-accent font-semibold tracking-wider transition-colors">
                          {subItem.name}
                        </Link>
                      ))}
                    </div>
                  ...
                  <Link key={item.title} href={item.url ?? "#"} onClick={closeMenu} className="hover:text-foxy-accent transition-colors">
                    {item.title}
                  </Link>
                )
              ))}
